...
async function isAuthenticated(token) {
    try {
        ...
        if(error.name == 'TokenExpiredError') {
            throw new AppError("Expiry JWT token", StatusCodes.BAD_REQUEST);
        }
        throw new AppError("Something went wrong", StatusCodes.INTERNAL_SERVER_ERROR);
    }
}
...
async function addRoleToUser(data) {
    try {
        ...
        if(!user){
            throw new AppError("User not found for the given name", StatusCodes.NOT_FOUND);
        }
        const role = await roleRepo.getRoleByName(data.role);
        if(!role){
            throw new AppError("User not found for the given role ", StatusCodes.NOT_FOUND);
        }
        user.addRole(role);
        return user;
